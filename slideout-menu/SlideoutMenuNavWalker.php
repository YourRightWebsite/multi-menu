...
class SlideoutMenuNavWalker extends \Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth=0, $args=null, $id=0) { 
            ...
            $output .= "
                <li class='multi-menu js-multi-menu-slideout multi-menu-slideout ". $this->css_classes ." multi-menu-slideout-" . $this->params['id'] . " " . $menu_invert_class . "'>
                    ...
                    <div class='menu-slideout-container'>
                        ...
                        <div class='menu-slideout-container-close-container'>
                            <button class='js-multi-menu-slideout-toggle-close'>
                                ". $menu_button_close_text ."
                                <span class='button-icon'>
                                    <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' stroke-width='1.5' stroke='currentColor' class='size-6'>
                        ...
                        <ul class='menu-slideout-container-inner'>
		...
		if ($item->url && $item->url != '#') {
			$output .= '<a class="menu-clickable depth-'. $depth . ' ' . ($args->walker->has_children ? 'has-children' : 'no-children') . '" href="' . $item->url . '">';
		} else {
			$output .= '<button class="menu-clickable js-menu-clickable-toggle is-button depth-'. $depth . ' ' . ($args->walker->has_children ? 'has-children' : 'no-children') . '">';
		}
